...
export function testBisectionInterval(
  ...
  const response: testBisectionIntervalResults = {
    f_a: 0,
    f_a_sign: null,
    f_b: 0,
    f_b_sign: null,
 ...
 */
export function calcBisectionForMantarie(
  a: number,
  b: number,
  funcType: functionTypeEnums,
  customFunc: string,
  ...
  while (hasReached0 == false ){
    ...
    // If the error is 0, break out of the loop
    // if (temp_less_than_error) {
    //   break;
    // }

    if (temp_d === 0 || temp_e === 0){
      hasReached0 = true;
      break;
    }
  }
